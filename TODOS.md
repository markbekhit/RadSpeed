...
- [x] **Thread safety in recorder.py** — `audio_data` and `paused` now use a shared state lock; chunks crossing pause/stop boundaries are discarded and post-processing receives a stable snapshot. Regression tests cover paused, stopped and snapshot behaviour.
