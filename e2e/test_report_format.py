...
@pytest.mark.parametrize('slug', ['ct-head-brain', 'ultrasound-doppler-venous'])
...
def test_denied_copy_selects_text_without_false_success(page, base_url):
    page.add_init_script("""Object.defineProperty(navigator, 'clipboard', {
        value: {writeText: async () => { throw new Error('denied'); }}
    });""")
    page.goto(f'{base_url}/report-templates/ct-head-brain')
    page.get_by_role('button', name='Copy blank report').click()
